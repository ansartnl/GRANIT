...
#include "src/dialogs/LogCollectorDialog.h"
      ...
      mIsStatusOn(false), mModifyFlag(false)
{
    ...
    connect(mConnection, SIGNAL(gotVariant(QVariant)), SLOT(on_variantReceived(QVariant)), Qt::QueuedConnection);

    int reconnectionTimeout = 10 * 1000;//xmlSettings().value("reconnection_timeout", 10).toInt() * 1000;
    ...
    connect(this, SIGNAL(error(QAbstractSocket::SocketError)), SLOT(on_error()));


    QScriptEngine* engine = scriptEngine();
    QScriptValue scriptWidget = engine->newQObject(this);
    engine->globalObject().setProperty(mId, scriptWidget);
}
...
void MonitoringSocket::changeStatus(bool isOn)
{
    ...
    {
        mConnector->addAddress(lookupHost(mHost), mPort);
        if (!mHost2.isEmpty())
            mConnector->addAddress(lookupHost(mHost2), mPort);
...
void MonitoringSocket::on_error()
{
    const bool hadError = property("had_error").toBool();
    if (hadError)
        return;
    setProperty("had_error", true);
    qLogWarn(tr("Monitoring")) << tr(" Monitoring socket '%1' error: %2").arg(mName).arg(errorString());
}
...
QHostAddress MonitoringSocket::lookupHost(const QString &host)
{
    ...
    if (!hostInfo.addresses().isEmpty()) {
        return hostInfo.addresses().first();
    }
...
void MonitoringSocket::on_variantReceived(const QVariant &v)
{
    ...
    else if ( v.canConvert<SupervisorProto::ScriptString>() )
    {
        ...
        SupervisorProto::RemoteLog log = v.value<SupervisorProto::RemoteLog>();
        emit remoteLogRecieved(QString("%1/%2/").arg(host()).arg(name()), log);
    }
...
bool MonitoringSocket::isAllProcessesOn() const
{
    foreach ( const MonitoredObject &obj, mObjects )
    {
        if ( !obj.isOn )
...
{
    qLogInfo(tr("User")) << "Start script '" << name << "' with parameter '" << parameter << "' requested on " << mId;
    mScriptOutput.clear();
    emit scriptOutputChanged();
    if ( state() == QAbstractSocket::ConnectedState )
    {
...
void MonitoringSocket::stopScript(const QString &name)
{
    if ( state() == QAbstractSocket::ConnectedState )
    {
        qLogInfo(tr("User")) << "Stop script requested on " << mId;
        SupervisorProto::StopScript a;
        a.name = name;
        mConnection->sendVariant( QVariant::fromValue(a) );
    }
}

void MonitoringSocket::snmp(const QString &args)
{
    QString agent = args.section('|', 0, 0);
    QString name  = args.section('|', 1, 1);
    QString value = args.section('|', 2, 2);

    if ( state() == QAbstractSocket::ConnectedState )
    {
        qLogInfo(tr("User")) << QString("Set SNMP variable '%1' to '%2' requested on %3:%4 ").arg(name).arg(value).arg(mId).arg(agent);
        SupervisorProto::SetSnmpVariable a;
        a.agent = agent;
        a.variable = name;
        a.value = value;
        mConnection->sendVariant( QVariant::fromValue(a) );
...
bool MonitoringSocket::shutdown()
{
    if ( state() == QAbstractSocket::ConnectedState )
    {
        qLogInfo(tr("User")) << "Shutdown requested on " << mId;
...
bool MonitoringSocket::reboot()
{
    if ( state() == QAbstractSocket::ConnectedState )
    {
        qLogInfo(tr("User")) << "Reboot requested on " << mId;
...
void MonitoringSocket::toggleStatus()
{
    changeStatus(!isStatusOn());

    if(isStatusOn())
    {
        qLogInfo(tr("User")) << "Turn status On requested on " << mId;
        emit socketConnected();
    }
    else
    {
        qLogInfo(tr("User")) << "Turn status Off requested on " << mId;
        emit socketDisconnected();
    }
}

void MonitoringSocket::requestRemoteLogs()
{
    if ( state() == QAbstractSocket::ConnectedState )
    {
        qLogInfo(tr("User")) << "Remote logs requested on " << mId;
        CLogCollectorDialog* dialog = new CLogCollectorDialog(supervisorMonitor());
