...
void OLDIProc::SendMessage(QSharedPointer<OLDIMessage> pMes, bool bWait/* = true*/)
{
    ...
    TVA_CATCHALL_TRY
    {
        ...
        //int lastid = query.GetLastInsertID().toUInt();
        transaction.commit();
...
void OLDIProc::OnAckTimer()
{
    ...
    TVA_CATCHALL_TRY
    {
        ...
        //                "SELECT msgid, msgtime, msgfrom, msgto, msgtext, msgtype "
        //                "FROM inputmessages "
        //                "WHERE msgto = :1 AND "+driver_time+" - msgtime < to_dsinterval(:2) "
        //                "ORDER BY msgtime");
